...
if not os.path.exists(path):
    os.makedirs(path)
with io.open('Apple WWDC/Data/Special Events/Third Party Events/ThirdPartyEvents_data.json', 'w', encoding='utf8') as outfile:
    ...
    for ele in Soup.find_all('p', {'class': 'description'}):
        events['description'].append(ele.text.replace("\t", "").replace("\n", ""))
